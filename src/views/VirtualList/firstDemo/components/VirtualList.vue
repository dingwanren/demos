...
<script lang="ts" setup>
  ...
  const getKey = computed(() => {
    return typeof props.itemKey === 'function'
      ? props.itemKey
      : (item: any) => item[props.itemKey as string];
  });
  ...
  const visibleCount = computed(() => {
    if (typeof props.itemSize === 'function') {
      ...
      let totalHeight = 0;
      while (
        totalHeight < containerHeight.value &&
        startIndex.value + count < props.listData.length
      ) {
        totalHeight += getItemSize.value(
          props.listData[startIndex.value + count]
        ) as number;
        count++;
      }
      return count;
  ...
  const totalHeight = computed(() => {
    if (typeof props.itemSize === 'function') {
      return props.listData.reduce(
        (sum, item) => sum + getItemSize.value(item),
        0
      );
    }
  ...
  const handleScroll = () => {
    ...
    if (typeof props.itemSize === 'function') {
      ...
      for (let i = 0; i < props.listData.length; i++) {
        sum += getItemSize.value(props.listData[i]) as number;
        if (sum >= scrollTop) {
          newStartIndex = i;
          break;
        }
      ...
      startIndex.value =
        Math.floor(scrollTop / props.itemSize) - props.bufferSize;
